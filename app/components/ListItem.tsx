...
function ListItem({ hex, rgb, luminance }: UploadedListItemProps) {
   ...
   return (
      <li className={`relative list-none w-[200px] h-[200px] flex items-center justify-center border rounded-full shadow-md transition-transform duration-300 ease-in-out hover:-translate-y-2 hover:ring-4 hover:ring-white`} style={{ background: rgb }}>
         <span
            className={`flex flex-col gap-[20px] items-center justify-between font-semibold text-[20px] cursor-pointer ${luminance > 0.5 ? 'text-slate-950' : 'text-slate-50'}`}
            onClick={(e) => {
               copyToClipboard(e);
               setCopied(true);
               setTimeout(() => {
